<h1 align="center">Eya Azzabi</h1>

<p align="center">
  <b>AI &amp; Data Engineer</b> · Tunisia<br>
  I build AI systems that reach real users — and study why they work.
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/eya-azzabi/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
  <a href="mailto:eyaazzabi04@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white" alt="Email"></a>
  <img src="https://img.shields.io/badge/Français-Avancé-blue?style=flat-square" alt="French">
  <img src="https://img.shields.io/badge/English-Advanced-blue?style=flat-square" alt="English">
  <img src="https://img.shields.io/badge/العربية-Native-blue?style=flat-square" alt="Arabic">
</p>

---

### About

I'm an **undergraduate engineering student at ESPRIT** (Data Science, graduating October
2027), completing a **Master's in Data Science** at the University of Carthage at the
same time — and working full-time as an **AI & Backend Developer**.

I took the second degree on by choice, and paid for it myself, because one wasn't
teaching me fast enough.

My background is mathematics before it was engineering, and that still shapes how I
work: I'd rather understand why a model behaves the way it does than watch a metric go
up. Day to day I build LLM agents, RAG pipelines, and computer-vision systems; on my own
time I go back to the statistics underneath them.

**I'm looking for a research internship in AI/ML** — especially where language models,
computer vision, or statistical modelling are pointed at a real scientific problem
rather than a product.

---

### Featured work

**🌡️ [PCA of Tunisian Temperatures](https://github.com/EyaAzzabi/pca-tunisia-temperatures)**
Given only twelve monthly averages per station and no geography whatsoever, PCA
reconstructed the two physical axes governing Tunisian climate — explaining **98.4%** of
variation across 16 stations. Component 1 turned out to be mean annual temperature
(r = 0.9996); component 2, the Mediterranean's moderating effect (r = 0.9893).
Extends my bachelor's thesis, supervised by Prof. Chaouki Aouiti.
`Python` `NumPy` `PCA`

**🦯 SenseMate** — IoT assistive technology for blind and visually impaired people.
Real-time object detection and identification with YOLO, built with **Enactus FSB** over
15 months as a social-impact project.
`PyTorch` `YOLO` `IoT` `Computer Vision`

**🩺 [PCOS Prediction](https://github.com/EyaAzzabi/Pcos_Prediction)**
Early detection of polycystic ovary syndrome from clinical and hormonal features.
Six models compared (Logistic Regression → XGBoost), SMOTE for class imbalance,
evaluated on ROC/AUC and F1. XGBoost came out ahead.
`scikit-learn` `XGBoost` `SMOTE` `Healthcare ML`

**📧 Email Fraud Detection** — binary classification with SVM and Random Forest over
TF-IDF features. **94% precision.**
`scikit-learn` `NLP` `TF-IDF`

**🤖 Smart Campus AI Assistant** — hackathon project: a locally-run chatbot plus a
lost-and-found assistant using CLIP for image retrieval.
`NLP` `CLIP` `LLM`

---

### Currently

- 🏢 Building AI backend services at **Mafrah** — LLM integration, data pipelines, APIs
- 🎓 Finishing **JobReady**, my Master's graduation project
- 🔬 Looking for a research internship for summer 2027

---

### Toolkit

| | |
|---|---|
| **Languages** | Python · SQL · R · Java · PL/SQL |
| **ML / DL** | PyTorch · scikit-learn · XGBoost · YOLO · CLIP · CNNs |
| **LLM / GenAI** | LangChain · RAG · FAISS · Chroma · agentic systems · tool-augmented AI |
| **Data** | pandas · NumPy · Spark · Hadoop · Snowflake |
| **Backend** | REST APIs · PostgreSQL · MySQL · MongoDB · Docker |
| **Stats / Viz** | PCA · inferential statistics · Matplotlib · Tableau · Power BI |

---

### Background

**École Supérieure Privée d'Ingénierie et de Technologies (ESPRIT)** — Engineering
Degree, Data Science · *2025–2027*
**Université de Carthage, FSB** — Master's in Data Science · *2024–present (concurrent)*
**Université de Carthage, FSB** — BSc Mathematics & Computer Science · *2021–2024*

**Mafrah** — AI & Backend Developer · *2025–present*
**Talan Tunisie** — AI Consultant Intern · *2025* — built agents for Connect'IS, an
AI-driven sales platform

Outside the code: sponsorship team at **Enactus FSB**, **Soliya Connect Program** alumna,
and speaker at **WYSE**.

---

<p align="center"><i>Arabic · French · English · a little German</i></p>
